...
static void dump_devmode_fields(const DEVMODEW *devmode)
{
    TRACE("dmFields=%s ", _DM_fields(devmode->dmFields));
    if (devmode->dmFields & DM_BITSPERPEL)
        TRACE("dmBitsPerPel=%d ", devmode->dmBitsPerPel);
    if (devmode->dmFields & DM_PELSWIDTH)
        TRACE("dmPelsWidth=%d ", devmode->dmPelsWidth);
    if (devmode->dmFields & DM_PELSHEIGHT)
        TRACE("dmPelsHeight=%d ", devmode->dmPelsHeight);
    if (devmode->dmFields & DM_DISPLAYFREQUENCY)
        TRACE("dmDisplayFrequency=%d ", devmode->dmDisplayFrequency);
    if (devmode->dmFields & DM_POSITION)
        TRACE("dmPosition=(%d,%d) ", devmode->u1.s2.dmPosition.x, devmode->u1.s2.dmPosition.y);
    if (devmode->dmFields & DM_DISPLAYFLAGS)
        TRACE("dmDisplayFlags=%d ", devmode->u2.dmDisplayFlags);
    if (devmode->dmFields & DM_DISPLAYORIENTATION)
        TRACE("dmDisplayOrientation=%d ", devmode->u1.s2.dmDisplayOrientation);
    TRACE("\n");
}
