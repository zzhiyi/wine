        ...
        DWORD flags, D3DADAPTER_IDENTIFIER8 *identifier)
{
    ...
    if (FAILED(hr = wined3d_output_get_adapter_ordinal(d3d8->wined3d, adapter, &wined3d_adapter)))
        return hr;
    ...
    if (SUCCEEDED(hr = wined3d_adapter_get_identifier(d3d8->wined3d, wined3d_adapter, flags, &adapter_id)))
    {
...
static HRESULT WINAPI d3d8_EnumAdapterModes(IDirect3D8 *iface, UINT adapter, UINT mode_idx, D3DDISPLAYMODE *mode)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_enum_modes(d3d8->wined3d, adapter, WINED3DFMT_UNKNOWN,
...
static HRESULT WINAPI d3d8_GetAdapterDisplayMode(IDirect3D8 *iface, UINT adapter, D3DDISPLAYMODE *mode)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_get_display_mode(d3d8->wined3d, adapter, &wined3d_mode, NULL);
    wined3d_mutex_unlock();
        ...
        D3DFORMAT adapter_format, DWORD usage, D3DRESOURCETYPE resource_type, D3DFORMAT format)
{
    ...
    if (format == D3DFMT_RESZ && resource_type == D3DRTYPE_SURFACE && usage == D3DUSAGE_RENDERTARGET)
    {
        ...
        hr = wined3d_adapter_check_device_format(d3d8->wined3d, wined3d_adapter, device_type, wined3dformat_from_d3dformat(adapter_format),
        ...
        D3DFORMAT adapter_format, D3DFORMAT rt_format, D3DFORMAT ds_format)
{
    ...
    hr = wined3d_adapter_check_depth_stencil_match(d3d8->wined3d, wined3d_adapter, device_type,
...
static HMONITOR WINAPI d3d8_GetAdapterMonitor(IDirect3D8 *iface, UINT adapter)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_get_desc(d3d8->wined3d, adapter, &desc);
    wined3d_mutex_unlock();
