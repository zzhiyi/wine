        ...
        const struct wined3d_adapter_ops *adapter_ops)
{
    ...
    struct wined3d_output *outputs, output;
    ...
    /* Put all outputs under the first adapter if the LUID is random */
    if (!luid && ordinal)
    {
        ret = TRUE;
        goto done;
    ...
    if (primary_idx)
    {
        ...
        adapter->outputs[primary_idx] = output;
    ...
    TRACE("adapter %p output count %d.\n", adapter, adapter->output_count);
    ret = TRUE;
done:
    if (!ret)
    {
...
{
    unsigned int adapter_idx, primary_index = 0;
    ...
    while ((adapter = wined3d_adapter_create(wined3d->adapter_count, flags)))
    {
        ...
        if (adapter->output_count && !lstrcmpW(adapter->outputs[0].device_name, primary_display))
            primary_index = wined3d->adapter_count;
