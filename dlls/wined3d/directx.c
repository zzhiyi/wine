...
static HRESULT wined3d_output_init(struct wined3d_output *output, struct wined3d_adapter *adapter,
        const WCHAR *device_name, UINT ordinal)
{
...
UINT CDECL wined3d_get_output_count(const struct wined3d *wined3d)
{
    TRACE("wined3d %p, reporting %u outputs.\n", wined3d, wined3d->output_count);
...
/* Note: dx9 supplies a format. Calls from d3d8 supply WINED3DFMT_UNKNOWN */
HRESULT CDECL wined3d_output_enum_modes(const struct wined3d *wined3d, UINT output_idx,
        enum wined3d_format_id format_id, enum wined3d_scanline_ordering scanline_ordering,
        UINT mode_idx, struct wined3d_display_mode *mode)
{
    ...
    TRACE("wined3d %p, output_idx %u, format %s, scanline_ordering %#x, mode_idx %u, mode %p.\n",
            wined3d, output_idx, debug_d3dformat(format_id), scanline_ordering, mode_idx, mode);

    if (!mode || output_idx >= wined3d->output_count)
        return WINED3DERR_INVALIDCALL;
    ...
    while (i <= mode_idx)
    {
        if (!EnumDisplaySettingsExW(wined3d->outputs[output_idx].device_name, j++, &m, 0))
        {
...
HRESULT CDECL wined3d_output_get_display_mode(const struct wined3d *wined3d, UINT output_idx,
        struct wined3d_display_mode *mode, enum wined3d_display_rotation *rotation)
{
    const struct wined3d_output *output;
    DEVMODEW m;

    TRACE("wined3d %p, output_idx %u, display_mode %p, rotation %p.\n",
            wined3d, output_idx, mode, rotation);
...
HRESULT CDECL wined3d_output_set_display_mode(struct wined3d *wined3d,
        UINT output_idx, const struct wined3d_display_mode *mode)
{
    struct wined3d_adapter *adapter;
    struct wined3d_output *output;
...
HRESULT CDECL wined3d_output_check_device_type(const struct wined3d *wined3d, UINT output_idx,
        ...
        enum wined3d_format_id backbuffer_format, BOOL windowed)
{
    ...
    TRACE("wined3d %p, output_idx %u, device_type %s, display_format %s, backbuffer_format %s, windowed %#x.\n",
            wined3d, output_idx, debug_d3ddevicetype(device_type), debug_d3dformat(display_format),
    ...
    if (!windowed)
    {
        ...
        if (!wined3d_output_get_mode_count(wined3d, output_idx,
    ...
    if (present_conversion)
    {
        ...
        if (FAILED(wined3d_output_check_device_format_conversion(wined3d, output_idx,
    ...
    for (i = 0; SetupDiEnumDeviceInfo(devinfo, i, &device_data) || adapter_fallback; ++i)
     {
        ...
        for (j = 0; enum_display_device(j, &state, buffer, sizeof(buffer)) || output_fallback; ++j)
        {
            ...
            sprintfW(buffer, display_fmt, j + 1);
            if (FAILED(wined3d_output_init(&wined3d->outputs[output_index], wined3d->adapters[i], buffer, output_index)))
                goto fail;
