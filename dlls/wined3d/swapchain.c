...
static HRESULT wined3d_swapchain_state_set_display_mode(struct wined3d_swapchain_state *state,
        struct wined3d *wined3d, unsigned int output_idx, struct wined3d_display_mode *mode)
{
    HRESULT hr;

    if (state->desc.flags & WINED3D_SWAPCHAIN_USE_CLOSEST_MATCHING_MODE)
    {
        if (FAILED(hr = wined3d_output_find_closest_matching_mode(wined3d, output_idx, mode)))
        {
            WARN("Failed to find closest matching mode, hr %#x.\n", hr);
        }
    }

    if (FAILED(hr = wined3d_output_set_display_mode(wined3d, output_idx, mode)))
    {
        WARN("Failed to set display mode, hr %#x.\n", hr);
        return WINED3DERR_INVALIDCALL;
    }
        ...
        struct wined3d *wined3d, unsigned int output_idx, const struct wined3d_display_mode *mode)
{
    ...
    TRACE("state %p, wined3d %p, output_idx %u, mode %p.\n", state, wined3d, output_idx, mode);
