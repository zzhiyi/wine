        ...
        struct wined3d_display_mode *mode, IUnknown *device)
{
    ...
    hr = wined3d_output_find_closest_matching_mode(output->adapter->factory->wined3d,
            output->wined3d_output_ordinal, mode);
        ...
        enum dxgi_mode_struct_version struct_version)
{
    ...
    *mode_count = max_count;
    ...
    {
        if (FAILED(hr = wined3d_output_enum_modes(wined3d, output->wined3d_output_ordinal,
...
static HRESULT STDMETHODCALLTYPE dxgi_output_GetDesc(IDXGIOutput4 *iface, DXGI_OUTPUT_DESC *desc)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_get_desc(output->adapter->factory->wined3d,
            output->wined3d_output_ordinal, &wined3d_desc);
    wined3d_mutex_unlock();
