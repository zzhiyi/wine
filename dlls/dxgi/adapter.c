...
static HRESULT dxgi_adapter_get_desc(struct dxgi_adapter *adapter, DXGI_ADAPTER_DESC3 *desc)
{
    ...
    if (FAILED(hr = wined3d_adapter_get_identifier(adapter->factory->wined3d, adapter->ordinal, 0, &adapter_id)))
        return hr;
        ...
        REFGUID guid, LARGE_INTEGER *umd_version)
{
    ...
    if (SUCCEEDED(hr))
        hr = wined3d_adapter_get_identifier(wined3d, adapter->ordinal, 0, &adapter_id);
        ...
        struct wine_dxgi_adapter_info *info)
{
    ...
    if (SUCCEEDED(hr = wined3d_adapter_get_identifier(adapter->factory->wined3d, adapter->ordinal, 0, &adapter_id)))
    {
