...
static void test_mode_change(IUnknown *device, BOOL is_d3d12)
{
     ...
     * calling SetFullscreenState(), SetFullscreenState() fails with DXGI_ERROR_NOT_CURRENTLY_AVAILABLE */
    change_ret = ChangeDisplaySettingsW(&devmode, CDS_UPDATEREGISTRY | CDS_NORESET);
    ok(change_ret == DISP_CHANGE_SUCCESSFUL, "ChangeDisplaySettingsW failed with %d.\n", change_ret);
    ...
    /* Test that mode restorations also happen for other outputs on fullscreen state changes */
    hr = IDXGIFactory_CreateSwapChain(factory, device, &swapchain_desc, &swapchain);
    ok(hr == S_OK, "CreateSwapChain failed, hr %#x.\n", hr);
    hr = IDXGISwapChain_SetFullscreenState(swapchain, TRUE, NULL);
    ok(hr == S_OK, "Got unexpected hr %#x.\n", hr);
    ...
    /* Test that mode restorations for other outputs use display settings in the registry */
    hr = IDXGIFactory_CreateSwapChain(factory, device, &swapchain_desc, &swapchain);
    ok(hr == S_OK, "CreateSwapChain failed, hr %#x.\n", hr);
    hr = IDXGISwapChain_SetFullscreenState(swapchain, TRUE, NULL);
    ok(hr == S_OK, "SetFullscreenState failed, hr %#x.\n", hr);
    ...
    /* Test that mode restorations also happen for other outputs on fullscreen state changes when
     * there are two fullscreen swapchains on different outputs */
    hr = IDXGIFactory_CreateSwapChain(factory, device, &swapchain_desc, &swapchain);
    ok(hr == S_OK, "CreateSwapChain failed, hr %#x.\n", hr);
