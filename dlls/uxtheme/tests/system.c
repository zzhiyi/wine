...
static void test_DrawThemeBackgroundEx(void)
{
    ...
    static const RECT rect = {0, 0, width, height};
    ...
    /* Drawing opaque background with transparent glyphs should discard the alpha values from the
     * glyphs */
    ...
    ok(hr == S_OK, "GetThemeBool failed, hr %#lx.\n", hr);
    ok(transparent, "Expected spin button glyph transparent.\n");
    DrawThemeBackgroundEx(htheme, mem_dc, SPNP_UP, UPS_NORMAL, &rect, NULL);
    ...
    /* Drawing transparent theme part background without glyphs should keep alpha values */
    CloseThemeData(htheme);
    ...
    ok(hr == E_PROP_ID_UNSUPPORTED, "GetThemeEnumValue failed, hr %#lx.\n", hr);

    memset(bits, 0xa5, width * height * sizeof(int));
    DrawThemeBackgroundEx(htheme, mem_dc, SBP_SIZEBOX, SZB_RIGHTALIGN, &rect, NULL);
    found = FALSE;
    ptr = bits;
    for (i = 0; i < width * height; ++i)
    {
    ...
    if (IsThemePartDefined(htheme, HP_HEADERDROPDOWN, 0))
    {
        ...
        memset(bits, 0xa5, width * height * sizeof(int));
        DrawThemeBackgroundEx(htheme, mem_dc, HP_HEADERDROPDOWN, HDDS_NORMAL, &rect, NULL);
        found = FALSE;
        ptr = bits;
        for (i = 0; i < width * height; ++i)
        {
