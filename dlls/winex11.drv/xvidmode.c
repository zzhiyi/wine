...
{
    INT depth_idx, mode_idx, offset, xf86vm_mode_count;
    ...
    {
        for (mode_idx = 0; mode_idx < xf86vm_mode_count; ++mode_idx)
        {
            offset = (sizeof(DEVMODEW) + sizeof(XF86VidModeModeInfo *)) * (depth_idx * xf86vm_mode_count + mode_idx);
            mode = (DEVMODEW *)((BYTE *)modes + offset);
            add_xf86vm_mode(mode, depths[depth_idx], xf86vm_modes[mode_idx]);
        }
    }
