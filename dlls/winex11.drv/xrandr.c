...
static BOOL xrandr14_get_gpus2( struct x11drv_gpu **new_gpus, int *count,
                                BOOL get_vulkan_properties, BOOL get_pci_id )
{
    ...
    for (i = 0; i < provider_resources->nproviders; ++i)
    {
        ...
        if (get_pci_id && !get_gpu_pci_id( &gpus[i], i ))
...
{
    return xrandr14_get_gpus2( new_gpus, count, TRUE, TRUE );
}
...
static BOOL xrandr14_get_id( const WCHAR *device_name, ULONG_PTR *id )
{
    ...
    if (!xrandr14_get_gpus2( &gpus, &gpu_count, FALSE, FALSE ))
        return FALSE;
...
static LONG xrandr14_set_current_settings( ULONG_PTR id, DEVMODEW *mode )
{
    ...
    screen_width = max((INT)screen_width, (INT)mode->u1.s2.dmPosition.x + (INT)mode->dmPelsWidth);
    screen_height = max((INT)screen_height, (INT)mode->u1.s2.dmPosition.y + (INT)mode->dmPelsHeight);
