...
static BOOL is_broken_driver(void)
{
    ...
    /* Check if only one mode is reported */
    ...
    /* Check if it's NVIDIA proprietary driver */
...
/* VkDisplayKHR should be automatically generated by dlls/winevulkan/make_vulkan.
 * But the VK_KHR_display extension is blacklisted as it needs WSI work */
VK_DEFINE_NON_DISPATCHABLE_HANDLE(VkDisplayKHR)

static BOOL get_gpu_luid( RRProvider provider_id, LUID *luid )
{
    ...
    {
        if (!SetupDiGetDevicePropertyW( devinfo, &device_data, &WINE_DEVPROPKEY_RANDR_PROVIDER_ID,
...
BOOL fill_vk_device_luid( VkPhysicalDevice physical_device,
...
#define LOAD_FUNCPTR(f)                                \
    if (!p##f && !(p##f = dlsym( vulkan_handle, #f ))) \
    {                                                  \
        ERR("Failed to load %s.\n", #f);               \
        return FALSE;                                  \
    }
...
#undef LOAD_FUNCPTR
    ...
    pvkGetRandROutputDisplayEXT = (void *)pvkGetInstanceProcAddr( vk_instance, "vkGetRandROutputDisplayEXT" );
    if (!pvkGetRandROutputDisplayEXT)
    {
    ...
    for (provider_idx = 0; provider_idx < provider_resources->nproviders; ++provider_idx)
    {
        ...
        {
            vr = pvkGetRandROutputDisplayEXT( physical_device, gdi_display,
                                              provider_info->outputs[output_idx], &vk_display);
            if (vr != VK_SUCCESS || vk_display == VK_NULL_HANDLE)
                continue;

            if (!get_gpu_luid(provider_resources->providers[provider_idx], &luid))
                goto done;
    ...
    if (screen_resources)
        pXRRFreeScreenResources( screen_resources );
    if (!ret)
        WARN("Failed to fill vulkan device LUID property.\n");
    return ret;
}
...
static BOOL xrandr14_get_gpus( struct x11drv_gpu **new_gpus, int *count )
{
    ...
    for (i = 0; i < provider_resources->nproviders; ++i)
    {
        ...
        gpus[i].randr_provider_id = provider_resources->providers[i];
