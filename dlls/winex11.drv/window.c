...
static inline BOOL is_window_resizable( struct x11drv_win_data *data, DWORD style )
{
    ...
    return is_window_rect_full_monitor( &data->whole_rect );
}
...
static BOOL CALLBACK enum_monitor_proc( HMONITOR monitor, HDC hdc, RECT *monitor_rect, LPARAM lparam )
{
    ...
    if ((info->rect->left <= monitor_rect->left &&
         info->rect->right >= monitor_rect->right &&
         info->rect->top <= monitor_rect->top &&
         info->rect->bottom >= monitor_rect->bottom))
    {
        info->full_monitor = TRUE;
        return FALSE;
    }
...
void update_net_wm_states( struct x11drv_win_data *data )
{
    ...
    if ((!data->fs_hack || fs_hack_enabled( monitor )) && is_window_rect_full_monitor( &data->whole_rect ))
    {
                                  ...
                                  const RECT *old_whole_rect, const RECT *old_client_rect )
{
    ...
    if (wm_is_kde( data->display ) && is_window_rect_full_monitor( &data->whole_rect ))
    {
