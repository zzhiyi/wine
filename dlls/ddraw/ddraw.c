        ...
        BOOL windowed, struct wined3d_swapchain **wined3d_swapchain)
{
    ...
    if (FAILED(hr = wined3d_output_get_display_mode(ddraw->wined3d, WINED3DOUTPUT_DEFAULT, &mode, NULL)))
    {
...
static HRESULT WINAPI ddraw7_RestoreDisplayMode(IDirectDraw7 *iface)
{
    ...
    if (SUCCEEDED(hr = wined3d_output_set_display_mode(ddraw->wined3d, WINED3DOUTPUT_DEFAULT, NULL)))
        ...
        DWORD bpp, DWORD refresh_rate, DWORD flags)
{
    ...
    if (SUCCEEDED(hr = wined3d_output_set_display_mode(ddraw->wined3d, WINED3DOUTPUT_DEFAULT, &mode)))
    {
        if (ddraw->primary)
        {
...
HRESULT ddraw_get_d3dcaps(const struct ddraw *ddraw, D3DDEVICEDESC7 *caps)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_adapter_get_device_caps(ddraw->wined3d, WINED3DADAPTER_DEFAULT, WINED3D_DEVICE_TYPE_HAL, &wined3d_caps);
    wined3d_mutex_unlock();
    if (FAILED(hr))
    {
...
static HRESULT WINAPI ddraw7_GetFourCCCodes(IDirectDraw7 *iface, DWORD *NumCodes, DWORD *Codes)
{
    ...
    {
        if (SUCCEEDED(wined3d_adapter_check_device_format(ddraw->wined3d, WINED3DADAPTER_DEFAULT, WINED3D_DEVICE_TYPE_HAL,
...
static HRESULT WINAPI ddraw7_GetMonitorFrequency(IDirectDraw7 *iface, DWORD *frequency)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_get_display_mode(ddraw->wined3d, WINED3DOUTPUT_DEFAULT, &mode, NULL);
    wined3d_mutex_unlock();
    if (FAILED(hr))
    {
...
static HRESULT WINAPI ddraw7_GetVerticalBlankStatus(IDirectDraw7 *iface, BOOL *status)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_get_raster_status(ddraw->wined3d, WINED3DOUTPUT_DEFAULT, &raster_status);
    wined3d_mutex_unlock();
    if (FAILED(hr))
    {
        ...
        DWORD *free)
{
    ...
    framebuffer_size = wined3d_adapter_calculate_format_pitch(ddraw->wined3d, WINED3DADAPTER_DEFAULT,
    ...
    if (total)
    {
        ...
        hr = wined3d_adapter_get_identifier(ddraw->wined3d, WINED3DADAPTER_DEFAULT, 0, &desc);
        ...
        DDSURFACEDESC2 *DDSD, void *Context, LPDDENUMMODESCALLBACK2 cb)
{
    ...
    {
        modenum = 0;
        while (wined3d_output_enum_modes(ddraw->wined3d, WINED3DOUTPUT_DEFAULT, checkFormatList[fmt],
        ...
        DDDEVICEIDENTIFIER2 *DDDI, DWORD Flags)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_adapter_get_identifier(ddraw->wined3d, WINED3DADAPTER_DEFAULT, 0x0, &adapter_id);
    wined3d_mutex_unlock();
        ...
        LPD3DENUMPIXELFORMATSCALLBACK callback, void *context)
{
    ...
    for (i = 0; i < ARRAY_SIZE(formats); ++i)
    {
        if (SUCCEEDED(wined3d_adapter_check_device_format(ddraw->wined3d, WINED3DADAPTER_DEFAULT, type,
    ...
    if (SUCCEEDED(wined3d_adapter_check_device_format(ddraw->wined3d, WINED3DADAPTER_DEFAULT, type, mode.format_id,
...
HRESULT ddraw_init(struct ddraw *ddraw, DWORD flags, enum wined3d_device_type device_type)
{
    ...
    if (FAILED(hr = wined3d_adapter_get_device_caps(ddraw->wined3d, WINED3DADAPTER_DEFAULT, device_type, &caps)))
    {
