...
static UINT WINAPI d3d9_GetAdapterModeCount(IDirect3D9Ex *iface, UINT adapter, D3DFORMAT format)
{
    ...
    wined3d_mutex_lock();
    ret = wined3d_output_get_mode_count(d3d9->wined3d, adapter,
        ...
        D3DFORMAT display_format, D3DFORMAT backbuffer_format, BOOL windowed)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_check_device_type(d3d9->wined3d, adapter, device_type, wined3dformat_from_d3dformat(display_format),
        ...
        D3DFORMAT format, BOOL windowed, D3DMULTISAMPLE_TYPE multisample_type, DWORD *levels)
{
    ...
    hr = wined3d_adapter_check_device_multisample_type(d3d9->wined3d, wined3d_adapter, device_type,
        ...
        D3DDEVTYPE device_type, D3DFORMAT src_format, D3DFORMAT dst_format)
{
    ...
    wined3d_mutex_lock();
    hr = wined3d_output_check_device_format_conversion(d3d9->wined3d, adapter, device_type,
...
static HRESULT WINAPI d3d9_GetDeviceCaps(IDirect3D9Ex *iface, UINT adapter, D3DDEVTYPE device_type, D3DCAPS9 *caps)
{
    ...
    hr = wined3d_adapter_get_device_caps(d3d9->wined3d, wined3d_adapter, device_type, &wined3d_caps);
    wined3d_mutex_unlock();
